...
const PassagensModule = (() => {
  ...
  function preencherFormularioPS(psData, barcoData, usuario) {
    ...
    const acordionsMergulho = document.querySelectorAll('[data-tipo-barco]');

    acordionsMergulho.forEach(acc => {
      if (tiposPermitidos.includes(tipoEmb)) {
        acc.classList.remove('accordion-item-hidden');
      } else {
        acc.classList.add('accordion-item-hidden');
      }
    });
